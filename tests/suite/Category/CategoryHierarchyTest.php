...
class CategoryHierarchyTest extends CategoryTestCase
{
    ...
    public function testWithoutNodeScope()
    {
        ...
        $this->assertEquals($expected,
            $child->ancestorsAndSelf()->withoutNode($this->categories('Child 2'))->get()->all());
    }
    ...
    public function testLimitDepthScope()
    {
        ...
        $this->assertEquals(array_slice($descendancy, 0, 3),
            $node->descendants()->limitDepth(3)->pluck('id')->toArray());
        $this->assertEquals(array_slice($descendancy, 0, 5),
            $node->descendants()->limitDepth(5)->pluck('id')->toArray());
        $this->assertEquals(array_slice($descendancy, 0, 7),
    ...
    {
        (new CategorySeeder())->nestUptoAt($this->categories('Child 2.1'), 3);
    ...
    public function testGetImmediateDescendants()
    {
        ...
        $this->assertEquals([$this->categories('Child 2.1')],
            $this->categories('Child 2')->getImmediateDescendants()->all());
    ...
    public function testToHierarchyNestsCorrectlyWithOrder()
    {
        ...
        $this->assertArraysAreEqual($expectedSubtreeZ,
            hmap($this->categories('Root Z', 'OrderedCategory')->getDescendantsAndSelf()->toHierarchy()->toArray()));
    }

    public function testGetNestedList()
    {
        $separator = ' ';
        $nestedList = Category::getNestedList('name', 'id', $separator);

        $expected = [
            1 => str_repeat($separator, 0) . 'Root 1',
            2 => str_repeat($separator, 1) . 'Child 1',
            3 => str_repeat($separator, 1) . 'Child 2',
            4 => str_repeat($separator, 2) . 'Child 2.1',
            5 => str_repeat($separator, 1) . 'Child 3',
            6 => str_repeat($separator, 0) . 'Root 2',
        ];
